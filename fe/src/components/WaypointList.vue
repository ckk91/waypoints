...
export default {
    name:'WaypointList',
    computed: {
        items() {return this.$store.state.waypoints},
        totalRows() {return this.$store.state.wpTable.totalRows},
        currentPage: {
            get(){
    ...
    methods: {
    onPaginationInput() {
          getWaypointPage(this.$store.state.wpTable.currentPage, this.$store.state.wpTable.itemsPerPage)
          .then(res => {
